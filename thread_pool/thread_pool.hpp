...
private:
    ...
    using Thread_info = std::pair<std::thread::id, bool>;
...
void ThreadPool::del_threads()
{
	m_pool.remove_if([&](Thread_info& th) { return th.second == false; });
}
...
void ThreadPool::set_num_threads(int n_threads)
{
    ...
    if (threads_to_add > 0) {
        ...
        for(int i = 0; i < threads_to_add; ++i){
			...
			m_pool.push_back(std::make_pair(new_thread.get_id(), true));
			new_thread.detach();
        }
        ...
        std::unique_lock<std::mutex> locker(m_mtx);
        m_cv.wait(locker, [&](){ return m_n_threads == n_threads; } );
        del_threads();
    }
}
